...
// Since this is the landing page, we use a separate void layout if needed,
// but for now we'll just force it to take up the whole screen.
definePageMeta({ layout: false })

const { x, y } = useMouse()
const { width, height } = useWindowSize()

// Calculate physics depth. Range goes from -1 to 1 based on mouse relative to center.
...
<template>
  <div
    ...
    <!-- 3D Parallax Skyline Scene -->
    <div class="absolute inset-0 pointer-events-none z-0">

      <!-- Layer 1: The Sky & Cybersun (Furthest, moves slowest) -->
      <div
        ...
        <!-- The Cyber Sun -->
        <div class="absolute top-[20%] left-[60%] w-[400px] h-[400px] bg-cyan-500/10 rounded-full blur-[80px]"></div>
        <div class="absolute top-[30%] left-[65%] w-[150px] h-[150px] bg-cyan-400/30 rounded-full blur-[40px]"></div>
      </div>

      <!-- Layer 2: Distant Grid / Horizon (Slightly faster) -->
      <div class="absolute inset-0 opacity-40 transition-transform duration-75 ease-out"
        ...
        <div class="absolute bottom-0 w-full h-[40vh] skyline-distant border-t border-cyan-900/50"></div>
      </div>

      <!-- Layer 3: Midground Tokyo Skyline Blocks (Faster, glowing edges) -->
      <div class="absolute inset-0 transition-transform duration-75 ease-out"
        ...
        <div
          ...
          <div class="absolute top-10 left-1/2 -translate-x-1/2 w-4 h-16 bg-cyan-500/20 blur-md animate-pulse"></div>
        </div>
        <div
        ...
        <div
          ...
          <div class="w-1 h-[40vh] bg-cyan-400/10 shadow-[0_0_15px_rgba(57,197,187,0.5)]"></div>
        </div>
        <div
      ...
      <!-- Layer 4: Foreground Pillars (Fastest, blurring frame) -->
      <div class="absolute inset-0 transition-transform duration-75 ease-out"
    ...
    <!-- Right-Bottom: Instance Information Panel -->
    <div
      ...
      <!-- Decor Ribbon -->
      <div class="absolute top-0 right-10 w-24 h-1 bg-cyan-500 shadow-[0_0_10px_#39C5BB]"></div>

      <div class="flex items-start justify-between mb-8">
        <div class="flex flex-col">
          <div class="flex items-center gap-3 mb-2">
            <AppLogo class="w-10 h-10 drop-shadow-[0_0_15px_rgba(57,197,187,0.8)]" />
            <h1 class="text-3xl font-black tracking-widest text-white drop-shadow-md">Asagity <span
                class="text-xs text-gray-500 font-bold ml-1 opacity-60">(ASGT)</span></h1>
          </div>
        ...
        <div
          ...
          <UIcon name="i-material-symbols-dns-outline"
      ...
      <div class="flex flex-col gap-4">
        ...
        <button @click.stop="showOnboarding = true"
          ...
          <div
            ...
            <span class="font-black tracking-widest text-white">→ 让我们开始吧 (START)</span>
    ...
    <!-- Onboarding Freedom Window (Free Window) -->
    <AppFreeWindow
      id="onboarding"
      v-slot="{}"
    ...
    >
      <div class="h-full flex flex-col md:flex-row overflow-hidden bg-black/40">
        ...
        <div class="md:w-3/5 p-8 flex flex-col border-b md:border-b-0 md:border-r border-white/10 relative overflow-hidden">
          <!-- Ambient glow -->
          <div class="absolute top-0 right-0 w-[200px] h-[200px] bg-cyan-500/10 rounded-full blur-[60px] pointer-events-none"></div>

          <div class="flex items-center gap-3 mb-8">
            <UIcon name="i-material-symbols-gpp-good" class="w-6 h-6 text-cyan-400" />
            <h2 class="text-xl font-black text-white tracking-wider">实例公约 (Covenant)</h2>
          </div>
        ...
        <div class="md:w-2/5 transition-colors md:bg-white/5 p-10 flex flex-col justify-center gap-5">
          ...
          <!-- Developer Bypass -->
          <NuxtLink to="/" @click.stop>
            <button class="w-full bg-transparent hover:bg-fuchsia-500/10 text-gray-400 hover:text-fuchsia-400 font-bold tracking-wider py-3 rounded-2xl border border-dashed border-gray-600 hover:border-fuchsia-500/50 transition-all text-xs flex items-center justify-center gap-2 group">
...
/* Mock illuminated building windows */
.skyline-windows {
