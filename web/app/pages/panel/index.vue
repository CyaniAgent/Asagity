    ...
    <div class="flex items-center justify-between mt-2 mb-2">
      <div class="flex items-center gap-3">
        <div class="w-12 h-12 rounded-full bg-cyan-500/10 flex items-center justify-center border border-cyan-500/20">
          <UIcon name="i-material-symbols-dashboard" class="w-7 h-7 text-cyan-500" />
        </div>
        <div class="flex flex-col">
          <h1 class="text-2xl font-black text-gray-900 dark:text-white tracking-wide">系统概览 (Sys Overview)</h1>
          <p class="text-[11px] font-bold text-gray-500 dark:text-gray-400">Asagity Backend Matrix • All Systems Go</p>
        </div>
      </div>
      <!-- Quick Actions -->
      <div class="flex gap-2">
        <UButton icon="i-material-symbols-download" label="导出日志" color="neutral" variant="soft" class="rounded-full font-bold hidden md:flex" />
        <UButton icon="i-material-symbols-refresh" color="primary" variant="solid" class="rounded-full shadow-md shadow-cyan-500/20" />
      </div>
    </div>
      ...
      <div v-for="m in metrics" :key="m.label" 
        ...
        <div class="flex justify-between items-start mb-4">
          <div :class="`w-10 h-10 rounded-2xl flex items-center justify-center ${m.bg}`">
            <UIcon :name="m.icon" :class="`w-6 h-6 ${m.color}`" />
          </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-2 bg-white/60 dark:bg-gray-800/60 backdrop-blur-xl rounded-[32px] border border-white/40 dark:border-gray-700/50 p-6 shadow-sm flex flex-col">
        <div class="flex justify-between items-center mb-6">
          <div class="flex flex-col">
            <h3 class="font-black text-lg text-gray-900 dark:text-white">网络流量与调度 (Network & Sync)</h3>
            <span class="text-xs font-bold text-gray-500">7-Day Aggregation</span>
          </div>
          <USelectMenu :options="['Last 7 Days', 'Last 30 Days', 'This Year']" model-value="Last 7 Days" class="w-32" />
        </div>
        <div class="flex-1 w-full min-h-[300px]">
          <ClientOnly fallback="Loading chart matrix...">
            <VChart :option="trafficOptions" autoresize class="w-full h-full" />
          </ClientOnly>
        </div>
      </div>
      ...
      <div class="bg-white/60 dark:bg-gray-800/60 backdrop-blur-xl rounded-[32px] border border-white/40 dark:border-gray-700/50 p-6 shadow-sm flex flex-col relative overflow-hidden">
        <!-- Decoration -->
        <div class="absolute -top-10 -right-10 w-32 h-32 bg-cyan-500/5 rounded-full blur-2xl"></div>
        <h3 class="font-black text-lg text-gray-900 dark:text-white mb-1">存储分布 (Storage Share)</h3>
        <span class="text-xs font-bold text-gray-500 mb-6">Volume Analysis</span>
        <div class="flex-1 w-full min-h-[250px]">
          <ClientOnly>
            <VChart :option="storageOptions" autoresize class="w-full h-full" />
          </ClientOnly>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="bg-white/60 dark:bg-gray-800/60 backdrop-blur-xl rounded-[32px] border border-white/40 dark:border-gray-700/50 shadow-sm p-6 flex flex-col">
        <div class="flex items-center justify-between mb-6">
          <h3 class="font-black text-lg text-gray-900 dark:text-white flex items-center gap-2">
            <UIcon name="i-material-symbols-list-alt" class="w-5 h-5 text-indigo-500" />
            系统日志 (Activity Log)
          </h3>
        ...
        <div class="flex flex-col gap-4 relative">
          ...
          <div v-for="(log, i) in adminLogs" :key="i" class="flex gap-4 relative z-10 w-full">
            <div class="w-8 h-8 rounded-full flex items-center justify-center shrink-0 border-[3px] border-white dark:border-gray-900"
      ...
      <div class="bg-white/60 dark:bg-gray-800/60 backdrop-blur-xl rounded-[32px] border border-white/40 dark:border-gray-700/50 shadow-sm p-6 flex flex-col">
        <h3 class="font-black text-lg text-gray-900 dark:text-white mb-6 flex items-center gap-2">
          <UIcon name="i-material-symbols-memory" class="w-5 h-5 text-fuchsia-500" />
          硬件脉搏 (Hardware Pulse)
        </h3>
        ...
        <div class="flex flex-col gap-6 flex-1 justify-center">
          ...
          <div class="flex flex-col gap-2">
            ...
            <div class="h-3 w-full bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden shadow-inner">
               <div class="h-full bg-gradient-to-r from-cyan-400 to-cyan-500 rounded-full w-[12%] animate-[pulse_2s_ease-in-out_infinite]"></div>
            </div>
          </div>
          ...
          <div class="flex flex-col gap-2">
            ...
            <div class="h-3 w-full bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden shadow-inner">
               <div class="h-full bg-gradient-to-r from-fuchsia-400 to-fuchsia-500 rounded-full w-[45%]"></div>
            </div>
          </div>
          ...
          <div class="flex flex-col gap-2">
            ...
            <div class="h-3 w-full bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden shadow-inner">
               <div class="h-full bg-gradient-to-r from-emerald-400 to-emerald-500 rounded-full w-[8%]"></div>
            </div>
          </div>
        </div>
      </div>
