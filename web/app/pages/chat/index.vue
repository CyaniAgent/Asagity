  ...
  <div class="flex flex-col gap-4 max-w-3xl mx-auto py-2 h-full">
    ...
    <div class="flex items-center justify-between text-gray-400 dark:text-gray-500 text-sm font-bold uppercase tracking-widest px-2 mb-2 border-b border-gray-200 dark:border-gray-800 pb-2">
      <span class="flex items-center gap-2">
        <UIcon
          name="i-material-symbols-history"
          class="w-4 h-4"
        />
        历史
      </span>
      <UButton
        icon="i-material-symbols-keyboard-arrow-up"
        color="neutral"
        variant="ghost"
        size="xs"
        class="rounded-full hover:bg-cyan-500/10 text-gray-400"
      />
    </div>
      ...
      >
        ...
        <div class="relative shrink-0">
          <UAvatar
            ...
            :alt="chat.name"
            size="xl"
            class="ring-2 ring-transparent group-hover:ring-cyan-500/50 transition-all duration-300 shadow-sm"
          />
          <!-- 状态点 (在线/新消息) -->
          <div
            v-if="chat.unread > 0"
            class="absolute -bottom-1 -right-1 w-5 h-5 bg-red-400 border-[2px] border-white dark:border-gray-800 rounded-full flex items-center justify-center pointer-events-none shadow-sm z-10"
          >
            <span
              v-if="chat.unread > 1"
              class="text-[10px] font-black text-white leading-none pt-[1px]"
            >{{ chat.unread }}</span>
            <span
              v-else
              class="w-2.5 h-2.5 bg-red-500 rounded-full mix-blend-screen"
            />
          </div>
          <div
            v-else
            class="absolute -bottom-1 -right-1 w-4 h-4 bg-yellow-400 border-[2px] border-white dark:border-gray-800 rounded-full pointer-events-none shadow-sm z-10"
          />
        </div>
        ...
        <div class="flex flex-col flex-1 min-w-0 overflow-hidden text-left py-0.5 justify-center">
          <div class="flex items-center gap-2 mb-1 whitespace-nowrap overflow-hidden text-ellipsis">
            ...
            <span
              v-if="chat.isGroup"
              class="text-[10px] font-black text-cyan-700 dark:text-cyan-300 bg-cyan-100 dark:bg-cyan-900/60 px-1.5 py-0.5 rounded-md self-center"
            >({{ chat.memberCount }}人)</span>
            <span class="text-[13px] font-semibold text-gray-400 dark:text-gray-500 shrink-0">{{ chat.username }}</span>
          </div>
          <p
            class="text-[14px] text-gray-600 dark:text-gray-300/80 font-medium truncate w-full"
            :class="chat.unread > 0 ? 'text-gray-900 dark:text-white font-bold' : ''"
          >
            {{ chat.lastMessage }}
          </p>
        </div>
