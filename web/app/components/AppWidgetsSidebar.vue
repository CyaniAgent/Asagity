  ...
  <div class="flex flex-col gap-6 p-6 h-full overflow-y-auto custom-scrollbar">
    ...
    >
      <h3 class="text-[13px] font-black tracking-widest text-gray-500 dark:text-gray-400 uppercase mb-4 flex items-center gap-1.5">
        <UIcon name="i-material-symbols-signal-cellular-alt" class="w-4 h-4 text-green-500" /> 在线情况
      </h3>
      <div class="flex items-center justify-between">
        ...
        <div v-if="onlineAvatars.length > 0" class="shrink-0">
          <UAvatarGroup size="sm" :max="4" class="ring-2 ring-white/50 dark:ring-gray-800/50 rounded-full shadow-sm">
            <UAvatar v-for="(user, idx) in onlineAvatars" :key="idx" :src="user.src" />
          </UAvatarGroup>
        </div>
      </div>
    </div>
    ...
    >
      <h3 class="text-[13px] font-black tracking-widest text-gray-500 dark:text-gray-400 uppercase mb-4 flex items-center gap-1.5">
        <UIcon name="i-material-symbols-trending-up" class="w-4 h-4 text-cyan-500" /> 新鲜的话题
      </h3>
      <template v-if="trendingTopics.length > 0">
        <div class="flex flex-col gap-3">
          <div v-for="(topic, idx) in trendingTopics" :key="idx" class="flex items-center justify-between group cursor-pointer">
            <div class="flex flex-col">
    ...
    >
      <h3 class="text-[13px] font-black tracking-widest text-gray-500 dark:text-gray-400 uppercase mb-4 flex items-center gap-1.5">
        <UIcon name="i-material-symbols-person-add" class="w-4 h-4 text-blue-500" /> 推荐关注
      </h3>
      <template v-if="recommendedUsers.length > 0">
        <div class="flex flex-col gap-4">
          <div v-for="(user, idx) in recommendedUsers" :key="idx" class="flex items-center gap-3 group">
            <UAvatar :src="user.avatar" size="md" class="transition-transform group-hover:scale-105" />
            ...
            <UButton icon="i-material-symbols-add" size="xs" color="primary" variant="soft" class="rounded-full w-8 h-8 flex items-center justify-center shrink-0" />
          </div>
        </div>
      </template>
      <template v-else>
    ...
    >
      ...
      <template v-if="federatedInstances.length > 0">
        <div class="flex flex-col gap-3">
          <div v-for="(instance, idx) in federatedInstances" :key="idx" class="flex items-center justify-between group cursor-pointer p-2 -mx-2 rounded-xl hover:bg-white/40 dark:hover:bg-gray-800/40 transition-colors">
            <div class="flex flex-col">
              ...
              <div class="flex items-center gap-1.5 mt-0.5">
                <span class="px-1.5 py-0.5 rounded-md text-[9px] font-black tracking-wider" :class="instance.protocol === 'ActivityPub' ? 'bg-orange-100 text-orange-600 dark:bg-orange-500/20 dark:text-orange-400' : 'bg-cyan-100 text-cyan-600 dark:bg-cyan-500/20 dark:text-cyan-400'">
                  {{ instance.protocol }}
                </span>
                <span class="text-[10px] font-bold text-gray-400 flex items-center gap-0.5">
                  <UIcon name="i-material-symbols-person" class="w-3 h-3" /> {{ instance.active }}
                </span>
              </div>
            </div>
            <UIcon name="i-material-symbols-chevron-right" class="w-4 h-4 text-gray-300 opacity-0 group-hover:opacity-100 transition-opacity transform group-hover:translate-x-1" />
          </div>
        </div>
      </template>
      <template v-else>
