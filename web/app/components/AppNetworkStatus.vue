...
// If we are on the root page, we consider it the Welcome/Login/Register page
...
<template>
  <Transition :name="isWelcomePage ? 'fade-slide-top' : 'fade-slide-bottom'">
    <div v-if="isOffline"
      ...
      <div
        class="bg-black/90 backdrop-blur-xl border-2 border-red-500/80 rounded-2xl px-6 py-4 shadow-[0_0_30px_rgba(239,68,68,0.3)] flex items-start gap-4 mx-4 max-w-xl pointer-events-auto cursor-pointer select-none active:scale-95 transition-transform"
        ...
        <div class="flex flex-col gap-1.5 pt-0.5">
          <span v-if="isWelcomePage" class="font-black text-red-100 text-sm tracking-widest drop-shadow-sm">
            Asagity NET 连接失败，目前无法提供任何在线服务（包括登录）
          </span>
          <span v-else class="font-black text-red-100 text-sm tracking-widest drop-shadow-sm">
            Asagity NET 连接失败，目前无法提供任何在线服务
          </span>

          <span v-if="isWelcomePage" class="text-xs text-red-400 font-bold tracking-wider leading-relaxed">
            请联系实例运营人员或稍后刷新重试
          </span>
        </div>
      </div>
