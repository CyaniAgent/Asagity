  ...
  <div class="h-screen w-screen flex bg-gray-100 dark:bg-gray-950 overflow-hidden font-sans">
    ...
    <aside class="w-64 h-full flex flex-col shrink-0 z-20">
      ...
      <div class="p-4 shrink-0">
        <UButton label="发布 (Publish)" color="primary" size="xl"
    ...
    <main class="flex-1 flex flex-col min-w-0">
      ...
      <header class="h-16 flex justify-between items-center px-6 shrink-0 z-10 transition-colors">
        ...
        <div class="flex items-center">
          <UNavigationMenu :items="tabs" class="w-full" />
        </div>
        ...
        <div class="flex items-center gap-4">
          <!-- 音乐播放器组件 (占位) -->
          <div
            class="flex items-center gap-2 bg-white/40 dark:bg-gray-800/40 backdrop-blur-md rounded-full pr-2 pl-1 py-1 border border-white/20 dark:border-gray-700/50 shadow-sm transition-all hover:scale-105 cursor-pointer">
            <div
              class="w-6 h-6 rounded-full bg-gradient-to-tr from-cyan-500 to-blue-500 animate-[spin_4s_linear_infinite] shrink-0 drop-shadow-sm" />
            <div class="w-24 overflow-hidden mask-image:linear-gradient(to_right,white_80%,transparent)">
              <div
                class="text-xs font-medium whitespace-nowrap animate-[marquee_10s_linear_infinite] inline-block text-gray-700 dark:text-gray-300">
                ♪ Tell Your World - kz(livetune)
              </div>
            </div>
            <UButton icon="i-lucide-play" color="neutral" variant="ghost" size="xs" class="rounded-full" />
          </div>
      ...
      <div class="flex-1 overflow-hidden p-4 pt-0">
        <div class="relative h-full w-full flex gap-4 overflow-hidden">
          <!-- 左侧：主视图容器 -->
          <div
            class="h-full bg-white dark:bg-gray-900 rounded-[30px] border border-gray-200/50 dark:border-gray-800/50 shadow-[0_4px_20px_rgba(0,0,0,0.03)] overflow-hidden transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)]"
            :class="[
              splitViewStore.isOpen ? 'w-1/2 -translate-x-2' : 'w-full translate-x-0'
            ]">
          ...
          <!-- 右侧：详情面板容器 -->
          <div class="h-full overflow-hidden transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)]" :class="[
            splitViewStore.isOpen ? 'w-1/2 opacity-100' : 'w-0 opacity-0 pointer-events-none'
          ]">
            <div
              class="h-full bg-white dark:bg-gray-900 rounded-[30px] border border-gray-200/50 dark:border-gray-800/50 shadow-[0_4px_20px_rgba(0,0,0,0.03)] overflow-hidden">
              <AppPostDetail v-if="splitViewStore.currentPost || splitViewStore.isOpen" />
            </div>
          </div>
        </div>
      </div>
