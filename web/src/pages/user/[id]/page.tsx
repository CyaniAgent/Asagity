...
export default function UserPage() {
  const { t } = useI18n();
  const { id } = useParams("/user/:id") as { id: string };
          ...
          {!loading && !error && user && (
            <>
              ...
              <div className="relative h-44 w-full overflow-hidden">
                {user.banner ? (
                  <img src={user.banner} className="absolute inset-0 w-full h-full object-cover" alt="Banner" />
                ) : (
                ...
                <div className="flex items-end justify-between -mt-12 relative z-10 mb-4">
                  <div className="relative">
                    <div className="w-24 h-24 rounded-full overflow-hidden ring-[5px] ring-white dark:ring-gray-950 shadow-lg">
                      {user.avatar ? (
                        <img src={user.avatar} width={96} height={96} className="w-full h-full object-cover" alt={user.displayName} />
                      ) : (
