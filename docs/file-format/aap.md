# AAP (Asagity Application)

> Asagity 应用生态系统规范文档（第三方 App 专用）

## 概述

**AAP (Asagity Application)** 是 Asagity 特有的第三方应用生态系统，基于 `.tar.gz` 构建。它允许第三方开发者以应用（App）的方式扩展 Asagity 平台的前端和后端功能。

> **术语边界（必读）**：Asagity 后端采用“模块 + App 环境”双层体系。**模块（Module）**仅指随服务端一起编译的官方核心模块（`Verse.Modules`）；**第三方扩展一律称为应用（App）**，不再称为“模块”，以 `.aap`（Asagity Application）包格式分发，运行在受控的 App 环境沙箱中。旧称 **AAP（Asagity Integrated Module）** 及其 `.aap` 扩展名已统一更名为 **AAP** 及其 `.aap` 扩展名；下文中的“模块”字样凡指第三方扩展的，均应理解为“应用”。

## 文件格式

AAP 应用文件使用 `.aap` 扩展名，本质上是一个 tar.gz 压缩包。
...
```json
{
  ...
  "registry": "https://aap.asagity.org/apps",
...
用于应用市场（AAP Store），未来可以做应用更新、下载、验证。
...
完整 URL 格式：
```
https://aap.asagity.org/apps/{name}/{version}/download
https://aap.asagity.org/apps/{name}/latest
https://aap.asagity.org/apps/{name}/versions
```
...
```
.aap 文件 → tarparser 解析 → 读取 manifest.json → 动态注册
```
...
// frontend/index.ts
export default defineAAPPlugin({
  name: 'music-player',
  setup() {
    // 注册组件
    registerComponents()
...
```
.aap 文件 → tarutil 解压 → 读取 manifest.json → 注册模块
```
...
1. **本地安装**：从文件系统安装 `.aap` 文件
2. **应用市场**：从 Asagity 应用市场下载
